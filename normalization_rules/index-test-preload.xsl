...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:marc="http://www.loc.gov/MARC21/slim">
  ...
  <!-- Pre-load all external documents for 100 -->
  <xsl:variable name="data-100-0" as="document-node()?" select="document(concat($base-url, '/100/0/0.xml'))"/>
  <xsl:variable name="data-100-1-d" as="document-node()?" select="document(concat($base-url, '/100/1/d.xml'))"/>
  <xsl:variable name="data-100-1-no-d" as="document-node()?" select="document(concat($base-url, '/100/1/no_d.xml'))"/>
  <xsl:variable name="data-100-3" as="document-node()?" select="document(concat($base-url, '/100/3/3.xml'))"/>
  
  <!-- Pre-load external document for 600 -->
  <xsl:variable name="data-600" as="document-node()?" select="document(concat($base-url, '/600/600.xml'))"/>
  
  <!-- Pre-load all external documents for 700 -->
  <xsl:variable name="data-700-1-d" as="document-node()?" select="document(concat($base-url, '/700/1/d.xml'))"/>
  <xsl:variable name="data-700-1-no-d" as="document-node()?" select="document(concat($base-url, '/700/1/no_d.xml'))"/>
  <xsl:variable name="data-700-0-3" as="document-node()?" select="document(concat($base-url, '/700/2_3/2_3.xml'))"/>
  ...
  <xsl:template match="datafield[@tag='100' or @tag='600' or @tag='700']">
    <xsl:copy>
      ...
      <!-- Select the appropriate pre-loaded external data -->
      <xsl:variable name="external-marc-data" as="document-node()?">
        <xsl:choose>
          <!-- Field 600 -->
          <xsl:when test="$field-tag = '600'">
            <xsl:sequence select="$data-600"/>
          </xsl:when>
          
          <!-- Field 100 -->
          <xsl:when test="$field-tag = '100' and $ind1 = '0'">
            <xsl:sequence select="$data-100-0"/>
          </xsl:when>
          <xsl:when test="$field-tag = '100' and $ind1 = '1' and $has-d">
            <xsl:sequence select="$data-100-1-d"/>
          </xsl:when>
          <xsl:when test="$field-tag = '100' and $ind1 = '1' and not($has-d)">
            <xsl:sequence select="$data-100-1-no-d"/>
          </xsl:when>
          <xsl:when test="$field-tag = '100' and $ind1 = '3'">
            <xsl:sequence select="$data-100-3"/>
          </xsl:when>
          
          <!-- Field 700 -->
          <xsl:when test="$field-tag = '700' and $ind1 = '1' and $has-d">
            <xsl:sequence select="$data-700-1-d"/>
          </xsl:when>
          <xsl:when test="$field-tag = '700' and $ind1 = '1' and not($has-d)">
            <xsl:sequence select="$data-700-1-no-d"/>
          </xsl:when>
          <xsl:when test="$field-tag = '700' and ($ind1 = '0' or $ind1 = '3')">
            <xsl:sequence select="$data-700-0-3"/>
          </xsl:when>
        </xsl:choose>
      </xsl:variable>
