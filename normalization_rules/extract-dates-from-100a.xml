...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <!--
    ...
    Match 100 fields that:
      ...
      • $a ends with YYYY- or YYYY-YYYY (that has period or comma and whitespace)
  ...
  <xsl:template match="
      datafield[@tag='100']
        ...
        [not(subfield[@code='d'])]
        [subfield[@code='a'][matches(normalize-space(.), '\d{4}-(\d{4})?[\.,]?\s*$')]]
    ...
    <xsl:copy>
      ...
      <!-- Extract dates and name (with optional trailing period or comma) -->
      <xsl:variable name="dates"
        select="replace($a, '^(.*?),?\s*(\d{4}-(\d{4})?)[\.,]?\s*$', '$2')"/>
      <xsl:variable name="name"
        select="replace($a, ',?\s*\d{4}-(\d{4})?[\.,]?\s*$', '')"/>
